...
def create_sim():
    # data = pd.read_csv('./Input_Movie_Recomendation_System/Outputs/preprocessed_4.csv')
    data = pd.read_csv('./outputs/preprocessed_4.csv')
...
def rcmd(m):
    ...
    if m not in data['movie_title'].unique():
        return('Sorry! The movie your requested is not available. Please check the spelling or try with some other movies')
    else:
...
def recommend():
    ...
    if type(r)==type('string'): # no such movie found in the database
        return render_template('recommend.html',movie=movie,r=r,t='s', suggestions = get_suggestions())
    else:
        ...
        return render_template('recommend.html',movie=movie,mtitle=r,t='l',cards=movie_cards,
            ...
            release_date=rd,status=status,runtime=runtime, suggestions = get_suggestions())                
